...
export class ContextEngine extends EventEmitter {
  ...
  private async ensureSubprocess(): Promise<void> {
    ...
    // Run from repo root so `python -m knowledge.graph` can import the package.
    // Derived from mdDbPath (always at <repoRoot>/md_db) — immune to whether
    // this file is loaded from source (knowledge/engine/) or dist (dist/knowledge/engine/).
    ...
    const proc = spawn(pythonExe, ["-m", "knowledge.graph"], {
    ...
    if (!started) {
      ...
      throw new Error(
        `Python knowledge.graph subprocess failed to start (exe: ${pythonExe}).` +
        `\nVerify the conda env exists: conda env create -f knowledge/graph/environment.yml` +
        stderrSummary,
      );
    }
  }
  ...
  private searchPythonPath(): string {
    ...
    throw new Error(
      ...
      "Ensure the environment exists: conda env create -f knowledge/graph/environment.yml"
    );
  }
  ...
  private async rpc(method: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    if (!this.subprocess?.stdin?.writable) {
      throw new Error(
        ...
        "Check ce_subprocess_log debug events or run: conda run -n obsidiclaw python -m knowledge.graph"
      );
    }
