...
class ShouldSkipEdgeTests(unittest.TestCase):
    ...
    def test_sideways_calls_without_overlap_skipped(self):
    ...
    def test_sideways_calls_with_overlap_not_skipped(self):
        # Title must use spaces so normalize_token splits into separate tokens.
        # "attention-layer" → single token "attention_layer" (no overlap),
        # but "Attention Layer" → {"attention", "layer"} which overlaps {"attention"}.
...
class ExpandGraphNeighborsTests(unittest.TestCase):
    ...
    def test_score_multiplied_by_edge_multiplier(self):
        ...
        seeds = [_seed("a", 1.0)]
        result = expand_graph_neighbors(_FakeGraphStore(triplets), parsed, seeds, set())
        self.assertAlmostEqual(result[0].score, 0.70)
    ...
    def test_defined_in_multiplier(self):
        ...
        result = expand_graph_neighbors(_FakeGraphStore(triplets), parsed, seeds, set())
        self.assertAlmostEqual(result[0].score, 0.80)
    ...
    def test_results_sorted_by_score_descending(self):
        triplets = [
            _FakeTriplet("a", "LINKS_TO", "b"),
            _FakeTriplet("a", "DEFINED_IN", "c"),  # 0.80 multiplier
        ]
