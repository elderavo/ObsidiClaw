...
- **Shared infrastructure**: `createObsidiClawStack()` in `entry/stack.ts` creates ContextEngine, RunLogger, NoteMetricsLogger, WorkspaceRegistry, SubagentRunner. Both `entry/extension.ts` (Pi TUI) and `agents/orchestrator/run.ts` (headless) consume it
...
## What Works
- Full stack: context injection, event logging, subagent tools — all via `createObsidiClawStack()` (no scheduler in stack)
