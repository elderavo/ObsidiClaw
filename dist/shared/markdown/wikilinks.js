 ...
 */
export function parseWikiLinks(content, sourceFile) {
    const links = [];
    const malformed = [];
 ...
 */
export function extractSimpleTargets(links) {
    const seen = new Set();
 ...
 */
export function isValidWikiLinkSyntax(linkText) {
...
// Internal
// ---------------------------------------------------------------------------
function parseWikiLinkInner(inner, raw, position, fullContent) {
    if (!inner)
        return null;
