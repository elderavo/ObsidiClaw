...
export class PruneClusterStorage {
    ...
    storeClusters(clusters) {
        const insertCluster = this.db.prepare(`INSERT OR REPLACE INTO prune_clusters
       (cluster_id, representative_note_id, size, max_similarity, min_similarity, avg_similarity)
       VALUES (?, ?, ?, ?, ?, ?)`);
        const insertMember = this.db.prepare(`INSERT OR REPLACE INTO prune_cluster_members
       (cluster_id, note_id, similarity, is_representative, status)
       VALUES (?, ?, ?, ?, ?)`);
        const tx = this.db.transaction(() => {
            for (const cluster of clusters) {
                insertCluster.run(cluster.clusterId, cluster.representativeNoteId, cluster.stats.size, cluster.stats.maxSimilarity, cluster.stats.minSimilarity, cluster.stats.avgSimilarity);
                for (const member of cluster.members) {
                    insertMember.run(cluster.clusterId, member.noteId, member.similarity, member.isRepresentative ? 1 : 0, member.status);
                }
            }
        });
        tx();
    ...
    listClusters(options) {
        ...
        const memberRows = statusFilter
            ? this.db
                .prepare("SELECT * FROM prune_cluster_members WHERE status = ? ORDER BY cluster_id, is_representative DESC, similarity DESC")
                .all(statusFilter)
            : this.db
                .prepare("SELECT * FROM prune_cluster_members ORDER BY cluster_id, is_representative DESC, similarity DESC")
                .all();
        const membersByCluster = new Map();
        for (const row of memberRows) {
            if (!membersByCluster.has(row.cluster_id))
                membersByCluster.set(row.cluster_id, []);
    ...
    getCluster(clusterId) {
        ...
        if (!cRow)
            return null;
        const mRows = this.db
            .prepare("SELECT * FROM prune_cluster_members WHERE cluster_id = ? ORDER BY is_representative DESC, similarity DESC")
            .all(clusterId);
        return this.toCluster(cRow, mRows);
    }
    updateMemberStatus(clusterId, noteId, status) {
        this.db
