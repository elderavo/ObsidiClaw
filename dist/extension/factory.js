            ...
            if (stack?.persistentBackend) {
                try {
                    ...
                    const specs = await (async () => {
                        try {
                            return (await import("../scheduler/persistent-tasks.js")).listTaskSpecs(paths.rootDir);
                        }
                        catch {
                            return [];
                        }
                    ...
                    const lines = ["[scheduler] persistent tasks:", ...specNames.map((n) => ` - ${n}`)];
        ...
        pi.registerTool({
            ...
            async execute() {
                try {
                    ...
                    const text = extractMcpText(result);
                    return { content: [{ type: "text", text }], details: { tool: "list_jobs", error: "" } };
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    return { content: [{ type: "text", text: `list_jobs failed: ${msg}` }], details: { tool: "list_jobs", error: msg } };
                }
            },
        });
        pi.registerTool({
            ...
            async execute(_id, { job_name }) {
                try {
                    ...
                    const text = extractMcpText(result);
                    return { content: [{ type: "text", text }], details: { job_name, error: "" } };
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    return { content: [{ type: "text", text: `run_job failed: ${msg}` }], details: { job_name, error: msg } };
                }
            },
        });
        pi.registerTool({
            ...
            async execute(_id, { job_name, enabled }) {
                try {
                    ...
                    const text = extractMcpText(result);
                    return { content: [{ type: "text", text }], details: { job_name, enabled, error: "" } };
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    return { content: [{ type: "text", text: `set_job_enabled failed: ${msg}` }], details: { job_name, enabled, error: msg } };
                }
            },
        });
        pi.registerTool({
            ...
            async execute(_id, params) {
                try {
                    ...
                    const text = extractMcpText(result);
                    return { content: [{ type: "text", text }], details: { name: params.name, interval_minutes: params.interval_minutes, error: "" } };
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    return { content: [{ type: "text", text: `schedule_task failed: ${msg}` }], details: { name: params.name, interval_minutes: params.interval_minutes, error: msg } };
                }
            },
        });
        pi.registerTool({
            ...
            async execute(_id, { name }) {
                try {
                    ...
                    const text = extractMcpText(result);
                    return { content: [{ type: "text", text }], details: { name, error: "" } };
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                    return { content: [{ type: "text", text: `unschedule_task failed: ${msg}` }], details: { name, error: msg } };
                }
            },
        });
