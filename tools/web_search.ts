...
// Provider factory
function createProvider(providerName: string): SearchProvider {
  switch (providerName) {
    case 'perplexity':
...
export async function webSearch(query: string, options: Partial<WebSearchConfig> = {}): Promise<SearchResult> {
  ...
  const config = WebSearchConfigSchema.parse({
    ...
    model: options.model || process.env.WEB_SEARCH_MODEL || 'llama-3.1-sonar-small-128k-online',
    maxTokens: options.maxTokens || (process.env.WEB_SEARCH_MAX_TOKENS ? Number(process.env.WEB_SEARCH_MAX_TOKENS) : 1000),
    temperature: options.temperature || (process.env.WEB_SEARCH_TEMPERATURE ? Number(process.env.WEB_SEARCH_TEMPERATURE) : 0.1),
  });
