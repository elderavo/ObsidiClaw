...
import { Settings, VectorStoreIndex, MetadataMode } from "llamaindex";
import { OllamaEmbedding } from "@llamaindex/ollama";
import { buildIndex } from "./indexer.js";
import type { ContextEngineConfig, ContextPackage, NoteType, RetrievedNote } from "./types.js";
...
export class ContextEngine {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `[context_engine] Indexing ${this.config.mdDbPath} ` +
 ...
 */
function formatContext(query: string, notes: RetrievedNote[]): string {
  if (notes.length === 0) {
  ...
  if (toolNotes.length > 0) {
    ...
    lines.push(
      "The following tools from the knowledge base may be relevant to this query.",
    );
