...
export class SqliteGraphStore {
  ...
  getNoteByPath(path: string): StoredNote | null {
    return (
      (this.db
        ...
        .get(path) as StoredNote | undefined) ?? null
    );
  }

  listAllNotes(): StoredNote[] {
    return this.db.prepare("SELECT * FROM notes").all() as StoredNote[];
  }
  ...
  replaceEdges(srcNoteId: string, dstNoteIds: string[]): void {
    ...
    const existing = this.db
      ...
      .all(...dstNoteIds) as { note_id: string }[];
    ...
    const insertStmt = this.db.prepare(
      "INSERT OR IGNORE INTO edges (src_note_id, dst_note_id) VALUES (?, ?)",
    );
   ...
   */
  getNeighbors(startIds: string[], maxDepth: number = 1): NeighborResult[] {
    if (startIds.length === 0 || maxDepth <= 0) return [];

    const forwardStmt = this.db.prepare(
      "SELECT dst_note_id AS neighborId FROM edges WHERE src_note_id = ?",
    );
    const backwardStmt = this.db.prepare(
      "SELECT src_note_id AS neighborId FROM edges WHERE dst_note_id = ?",
    );

    const results: NeighborResult[] = [];
    const visited = new Set<string>(startIds);

    type QueueItem = { noteId: string; depth: number; linkedFrom: string };
    const queue: QueueItem[] = startIds.map((id) => ({
      noteId: id,
      depth: 0,
      linkedFrom: "",
    }));
    ...
    while (queue.length > 0) {
      ...
      const forwardNeighbors = forwardStmt.all(current.noteId) as { neighborId: string }[];
      const backwardNeighbors = backwardStmt.all(current.noteId) as { neighborId: string }[];
  ...
  resolveLink(linkText: string): string | null {
    const rows = this.db
      .prepare(
        `SELECT note_id, note_type FROM notes
         WHERE note_id = ?
            OR note_id LIKE ?`,
      )
      .all(
        `${linkText}.md`,
        `%/${linkText}.md`,
      ) as { note_id: string; note_type: string }[];

    if (rows.length === 0) return null;
    if (rows.length === 1) return rows[0]!.note_id;

    // Multiple matches — prefer tool > concept > index
    const priority: Record<string, number> = { tool: 0, concept: 1, index: 2 };
    rows.sort(
      (a, b) =>
        (priority[a.note_type] ?? 3) - (priority[b.note_type] ?? 3),
    );
